...
#define Py_HMAC_HACL_UPDATE_CALL(HACL_STATE, BUF, LEN)          \
    _hacl_hmac_state_update(HACL_STATE, BUF, (uint32_t)(LEN))
...
typedef void *HACL_HMAC_state; // TODO: use the correct struct
...
#define _HMACObject_CAST(PTR)   ((HMACObject *)(PTR))
...
#else
#define assert_is_static_hmac_hash_kind(_KIND)
#endif
...
_hacl_hmac_state_new(HMAC_Hash_Kind kind, uint8_t *key, uint32_t len)
{
    ...
    // TODO: use agile API (for now, only a PoC)
    return state;
}
...
/*
 * Copy the HACL* internal state.
 *
 * Return 0 on success and -1 on failure.
 */
static int
_hacl_hmac_state_copy(HACL_HMAC_state **out, HACL_HMAC_state *state)
{
    assert(out != NULL);
    // TODO: use agile API & decide whether out should be a double pointer
    PyErr_SetNone(PyExc_NotImplementedError);
    return -1;
}

/*
 * Update the HACL* internal state.
 */
static hacl_errno_t
_hacl_hmac_state_update(HACL_HMAC_state *state, uint8_t *msg, uint32_t len)
{
    assert(state != NULL);
    PyErr_SetNone(PyExc_NotImplementedError);
...
/*
 * Compute the HMAC-HASH digest from the HACL* internal state.
 */
static hacl_errno_t
_hacl_hmac_state_digest(HACL_HMAC_state *state, uint8_t *digest)
{
    assert(state != NULL);
    assert(digest != NULL);
    PyErr_SetNone(PyExc_NotImplementedError);
...
#undef Py_HMAC_HINFO_HACL_API
    /* sentinel */
    {
        NULL, NULL, Py_hmac_kind_hash_unknown, 0, 0,
        {NULL, NULL},
        NULL,
        0,
    },
...
hmac_copy_state(HMACObject *out, const HMACObject *src)
{
    ...
    return _hacl_hmac_state_copy(&out->state, src->state);
}
 ...
 *
 * At least 'N := self->digest_size' bytes should be available
...
{
    assert(digest != NULL);
    hacl_errno_t code = _hacl_hmac_state_digest(self->state, digest);
    return _hacl_convert_errno(code, self->name);
}
...
{
    HMACObject *hmac = _HMACObject_CAST(self);
